...
get_rds_endpoint() {
    ...
    # Get stack outputs (adjust stack name as needed)
    local stack_name="PawfectMatch-${ENVIRONMENT}"
...
main() {
    ...
    if deploy_cdk; then
        ...
        # Install PostGIS
        echo
        read -p "Do you want to install PostGIS extension? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            install_postgis
        else
            print_info "Skipping PostGIS installation"
        fi
